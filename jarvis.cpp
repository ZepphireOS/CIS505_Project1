...
// Jarvis Algorithm Class
class JarvisAlgorithm {
public:
    int findLeftmost(const vector<Point>& points, int numPoints) {
        int leftmost = 0;
        ...
        for (int i = 1; i < numPoints; i++) {
            ...
            if (points[i].x < points[leftmost].x - EPSILON) {
                leftmost = i;
            } else if (fabs(points[i].x - points[leftmost].x) < EPSILON) {
                metrics.comparisons++;
                if (points[i].y < points[leftmost].y - EPSILON) {
                    leftmost = i;
                }
            }
        }
    ...
    int computeConvexHull(const vector<Point>& points, int numPoints, vector<Point>& hull) {
        if (numPoints < 3) return 0;
        
        int leftmostIndex = findLeftmost(points, numPoints);
        ...
        do {
            ...
            for (int j = 0; j < numPoints; j++) {
                metrics.comparisons++;
                int orient = orientation(points[currentPoint], points[j], points[nextPoint]);
                
                if (orient == 2) {
                    nextPoint = j;
                } else if (orient == 0) {
                    metrics.comparisons++;
                    double dist1 = euclideanDistance(points[currentPoint], points[j]);
                    double dist2 = euclideanDistance(points[currentPoint], points[nextPoint]);
...
public:
    ...
    double verifyHull(const vector<Point>& allPoints, const vector<Point>& hull) {
        ...
        for (const Point& p : allPoints) {
            ...
            // If not on hull, it should be inside
...
void printMetrics(const string& filename) {
    ...
    cout << "  Algorithm Time: " << metrics.algorithmTime << " ms\n";
    ...
    file << "  Preprocessing Time: " << fixed << setprecision(3) 
         << metrics.preprocessTime << " ms\n";
    file << "  Algorithm Time: " << metrics.algorithmTime << " ms\n";
...
int main(int argc, char* argv[]) {
    ...
    cout << "========================================\n";
    cout << "JARVIS CONVEX HULL ALGORITHM\n";
    cout << "========================================\n";
    ...
    vector<Point> hull;
    int hullSize = jarvis.computeConvexHull(points, numPoints, hull);
    ...
    metrics.peakMemoryKB = (sizeof(Point) * points.size() + sizeof(Point) * hull.size()) / 1024.0;
    ...
    jarvis.printResults(hull, hullSize);
    jarvis.writeResults(hull, hullSize, outputFilename);
